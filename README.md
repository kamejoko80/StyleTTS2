...
git clone git@github.com:kamejoko80/StyleTTS2.git
cd StyleTTS2
...
3. Run the bellow script to download the prebuilt checkpoint:
```bash
python - <<'PY'
...
os.makedirs("Models/LJSpeech", exist_ok=True)
...
4. Run the bellow script to download the config:
```bash
python - <<'PY'
from huggingface_hub import hf_hub_download
import os
...
```bash
(env) henry:StyleTTS2$ ls Models/LJSpeech/
config.yml epoch_2nd_00100.pth
```

6. Run the bellow script to download nltk resource:
```bash
python - <<'PY'
import nltk
nltk.download("punkt")
